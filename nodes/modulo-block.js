        ...
        node.on("input", function(msg, send, done) {
            ...
            if (node.operationMode === "map") {
                ...
                for (const mapping of node.mappings) {
                    ...
                    if (!numericValue.valid || (mapping.input > 1 && numericValue.value === 0)) {
                        utils.setStatusError(node, numericValue.valid ? "modulo by zero" : `invalid ${mapping.property}`);
                        if (done) done();
                        return;
                    }
                ...
                const result = node.inputs.reduce((acc, value, index) => index === 0 ? value : acc % value, node.inputs[0]);
                const statusText = `in: [${node.inputs.join(", ")}], remainder: ${result.toFixed(2)}`;
