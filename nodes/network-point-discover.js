...
module.exports = function(RED) {
    ...
    function resultsPage(result) {
        ...
        return `<!doctype html><html><head><meta charset="utf-8"><title>Network Point Discovery</title><style>body{font:14px sans-serif;margin:24px;color:#202124}h1{margin:0 0 6px}p{color:#5f6368;margin:0 0 20px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #dadce0;padding:8px;text-align:left}th{background:#f1f3f4}tr:nth-child(even){background:#fafafa}</style></head><body><h1>Network Point Discovery</h1><p>${rows.length} point record${rows.length === 1 ? "" : "s"} found${result?.timestamp ? ` at ${escapeHtml(new Date(result.timestamp).toLocaleString())}` : ""}.</p><table><thead><tr><th>Scope</th><th>Store</th><th>Registry</th><th>Path</th><th>Type</th><th>Point ID</th><th>Writable</th></tr></thead><tbody>${body}</tbody></table></body></html>`;
    }
