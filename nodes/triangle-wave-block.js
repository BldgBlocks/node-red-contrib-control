module.exports = function(RED) {
    function TriangleWaveBlockNode(config) {
        ...
        node.name = config.name || "triangle wave";
        ...
        node.on("input", function(msg, send, done) {
            ...
            const triangleValue = phase < 0.5 ? 2 * phase : 2 * (1 - phase);
    ...
    RED.httpAdmin.get("/triangle-wave-block/:id", RED.auth.needsPermission("triangle-wave-block.read"), function(req, res) {
        const node = RED.nodes.getNode(req.params.id);
        if (node && node.type === "triangle-wave-block") {
